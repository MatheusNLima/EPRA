...
// Configuração do diretório de upload para tutoriais
const uploadDir = path.join(__dirname, '../../uploads/tutoriais');
...
const uploadTutorialMiddleware = (req, res, next) => {
    // Permite no máximo 5 imagens sob o campo 'imagens'
    upload.array('imagens', 5)(req, res, (err) => {
        if (err) {
            console.error('❌ ERRO NO UPLOAD DE TUTORIAL:', err);
...
module.exports = uploadTutorialMiddleware;
