...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adicionado ALUNO_INTERNO para suportar as atualizações da equipa
...
window.carregarDesafiosAdmin = async function() {
    ...
    try {
        ...
        if (desafios.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center;">Nenhum desafio cadastrado ainda.</td></tr>';
            return;
        }
        ...
        desafios.forEach(desafio => {
            ...
            if (desafio.datalimite) {
                ...
                prazo = dateObj.toLocaleDateString('pt-BR');
            }
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>${prazo}</td>
                    <td>
                        <button class="btn-sm btn-info" onclick="verSolucoes(${desafio.id})">💡 Soluções</button>
        ...
        tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: red;">Erro ao carregar desafios.</td></tr>';
    }
};

window.salvarNovoDesafio = async function() {
    const form = document.getElementById('formDesafio');
    if (!form.reportValidity()) return;
...
window.salvarEdicaoDesafio = async function() {
    const form = document.getElementById('formEditarDesafio');
    if (!form.reportValidity()) return;
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex; flex-direction:column; gap:5px;">
                            ...
                            <button class="btn-sm btn-info" onclick="salvarNotaSolucao(${sol.id}, ${desafioId})" style="margin-top: 5px;">Salvar</button>
                        </div>
                    </td>
                </tr>
            `;
